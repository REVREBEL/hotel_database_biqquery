...
-- clean_crm_schema_v3.sql
-- Portable PostgreSQL schema (no prefixes). Removes all "ALTER TABLE ... IF NOT EXISTS".
-- Uses DO-block checks for idempotent FK creation. Safe to run multiple times.
...
-- Add FK leads.campaign_id -> campaigns.id (idempotent via DO block)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    WHERE t.relname = 'leads' AND c.conname = 'fk_leads_campaign'
  ) THEN
    ALTER TABLE leads
...
-- Add FK files.owner_project_id -> projects.id (idempotent via DO block)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint c
    JOIN pg_class t ON t.oid = c.conrelid
    WHERE t.relname = 'files' AND c.conname = 'fk_files_project'
  ) THEN
    ALTER TABLE files
